  ...
  return (
    <section className="settings-panel developer-tools-panel">
      <div className="settings-panel__heading">
        ...
        <div>
          <h2>{t("개발 도구 관리")}</h2>
          <p>{t("Docker를 사용하는 경우에만 켜세요.")}</p>
        </div>
      </div>

      <label className="setting-row developer-tool-toggle">
        <span>
          <strong>{t("Docker 용량 관리")}</strong>
          <small>
            {t("켜면 사이드바에 Docker 전용 메뉴가 나타납니다. 대시보드에는 표시하지 않습니다.")}
          </small>
        </span>
      ...
      {showWorking ? (
        <div className="docker-tool-state" role="status">
          ...
          <span>{changing ? t("Docker 설정 적용 중…") : t("Docker 설정 확인 중…")}</span>
        </div>
      ) : null}
      ...
      {!loading && status?.enabled ? (
        <div className="docker-settings-link-row">
          <span>
            <strong>{t("Docker 용량 메뉴가 켜졌습니다")}</strong>
            <small>{t("상태 확인과 정리 검토는 전용 화면에서 진행합니다.")}</small>
          </span>
