    ...
    <div className="view-stack dashboard-view">
      ...
      <section className="dashboard-panel dashboard-volumes" aria-labelledby="dashboard-volumes-title">
        ...
        {volumes.length > 0 ? (
          ...
          <DashboardEmpty
            title={loading ? "드라이브를 확인하고 있습니다" : "드라이브 정보를 읽지 못했습니다"}
            detail="새로 고침한 뒤에도 보이지 않으면 운영체제 권한을 확인해 주세요."
            actionLabel="다시 확인"
            onAction={onRefresh}
          />
        )}
      ...
      <div className="dashboard-activity-grid">
        <section className="dashboard-panel dashboard-history" aria-labelledby="dashboard-history-title">
          ...
          {actionHistory?.entries.length ? (
            ...
            <DashboardEmpty
              ...
              detail="파일을 선택하고 최종 확인한 뒤 휴지통으로 옮긴 결과만 여기에 남습니다."
              actionLabel="용량 관리 열기"
        ...
        <section className="dashboard-panel dashboard-recent" aria-labelledby="dashboard-recent-title">
          <DashboardHeading
            title="최근 추가된 파일"
            detail="BroomSweepy가 이전 목록 이후 새로 발견"
          ...
          {!fileCatalog || !recentFiles ? (
            <DashboardEmpty
              title="먼저 파일 목록을 만들어 주세요"
              detail="첫 목록은 비교 기준으로만 저장하며 기존 파일을 모두 새 파일로 표시하지 않습니다."
              actionLabel="파일 목록 만들기"
              onAction={onRefreshFileCatalog}
              disabled={blocked}
            />
          ) : !recentFiles.comparisonReady ? (
            <DashboardEmpty
              title="비교 기준 목록이 준비됐습니다"
              detail={`마지막 목록 ${formatDate(recentFiles.completedAtUnixMs)}. 다음 갱신부터 새로 발견한 파일을 표시합니다.`}
              actionLabel="목록 새로 고침"
              onAction={onRefreshFileCatalog}
              disabled={blocked}
            />
            ...
            <>
                  ...
                  >
                    <span>
                      ...
                      <small title={file.parent}>
                        {file.parent} · {formatDate(file.firstSeenAtUnixMs)} 발견
                      </small>
                    </span>
              ...
              <div className="dashboard-list-footer">
                <span>
                  새 파일 {formatCount(recentFiles.totalNewFiles)}개
                  {recentFiles.resultsTruncated ? " · 최근 항목만 표시" : ""}
                </span>
            ...
            <DashboardEmpty
              title="이전 목록 이후 새 파일이 없습니다"
              detail={`마지막 비교 ${formatDate(recentFiles.completedAtUnixMs)}`}
              actionLabel="목록 새로 고침"
              onAction={onRefreshFileCatalog}
              disabled={blocked}
            />
  ...
  return (
    <div className={`dashboard-volume-row ${warning ? "has-warning" : ""}`}>
      <span className="dashboard-volume-name">
        <strong>{label}</strong>
        <small>
          {volume.fileSystem || "파일 시스템 정보 없음"}
          {volume.isSystem ? " · 시스템" : ""}
          {volume.removable ? " · 이동식" : ""}
        </small>
      </span>
  ...
  return (
    <div className="dashboard-history-row">
      ...
      <span>
        <strong>{title}</strong>
        <small>
          요청 {formatCount(entry.requestedCount)}개 중 {formatCount(entry.movedCount)}개 이동 · {status}
        </small>
      </span>
