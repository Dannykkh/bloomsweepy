...
- 기본 전역 메뉴는 `대시보드`, `공간 정리`, `파일 관리`, `성능`, `AI 도우미`, `설정`이다. 기존 leaf route는 공간 정리와 파일 관리의 보조 탭으로 보존한다. 설정에서 Docker 관리를 켠 경우에만 AI 도우미 뒤에 `Docker 관리`를 추가한다.
...
- Treemap actions: 폴더 클릭은 하위 탐색, 파일 클릭은 파일 관리자에서 위치 열기다. 같은 경로의 블록과 순위는 hover·keyboard focus 시 함께 강조한다. 순위 행의 44px 더보기와 우클릭 메뉴에서 추가 작업을 제공하며 파일 한 개만 별도 확인 후 휴지통으로 이동한다. 저장공간 헤더는 일반 문서 흐름으로 두고 드릴다운마다 자동 스크롤하지 않는다.
