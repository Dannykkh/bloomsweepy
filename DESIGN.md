...
BroomSweepy는 저장공간을 관찰하고 안전하게 정리하는 데스크톱 계기판이다. 기존 SwiftUI 앱의 어두운 글래스 재질과 저장공간 링을 보존하되, 장식적인 글래스모피즘보다 스캔 상태와 안전 판단을 먼저 읽히게 한다. 시각적으로 감수하는 한 가지 위험은 파란색에서 보라색으로 이어지는 기존 브랜드 스윕이며, 중앙 저장공간 링과 선택 상태에만 제한한다.
...
한글과 라틴을 한 목소리로 유지하기 위해 본문과 제목 모두 실제 로드한 Pretendard Variable을 사용한다. 경로, 크기, 진행률처럼 자릿수 비교가 중요한 값만 JetBrains Mono를 쓴다. 대시보드이므로 serif와 과도한 디스플레이 타이포를 사용하지 않는다.

## Spatial Model

- 넓은 창: 216px 고정 사이드바 + 유연한 메인 작업면.
- 메인 첫 화면: 상태 브리핑 → 저장공간 링과 주 행동 → 최근 스캔 요약과 큰 항목 목록.
- 카드의 반복보다 얇은 선과 명도 차를 우선한다. 중앙 저장공간 스테이지만 강한 유리 셸을 사용한다.
...
- 키보드 포커스는 2px 외곽선으로 표시하고 색 대비만으로 상태를 구분하지 않는다.
