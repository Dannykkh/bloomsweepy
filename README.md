...
- 설정에서 Windows·macOS 로그인 시 자동 시작을 직접 등록·해제하고, 변경 뒤 운영체제의 실제 상태를 다시 확인
- 자동 시작은 기본적으로 꺼져 있으며 `--background`로 창을 처음부터 숨김. 일반 중복 실행은 새 프로세스를 남기지 않고 기존 창을 복원
- 전체·사용 가능·사용 중 메모리와 플랫폼이 보고하는 스왑 지표를 읽기 전용으로 표시
- Windows 스왑은 실제 페이지 파일 현재 사용량이 아닌 커밋 기반 추정치로 구분하고, Working Set·다른 앱·누수 메모리 정리 기능은 제공하지 않음
...
- 사용자가 설정에서 직접 등록·해제하고 실제 운영체제 상태를 다시 확인하는 Windows·macOS 자동 시작. 기본값은 꺼짐이며 자동 시작 때만 `--background`로 조용히 실행
- 전체·사용 가능·사용 중 메모리와 `sysinfo`가 보고하는 swap 지표만 읽는 시스템 상태. Windows swap 값은 commit 기반 추정치이며 현재 페이지 파일 사용량이 아님. Working Set·standby 캐시·다른 앱·누수 메모리는 정리하지 않음
...
`v1.5.0`의 자동 시작은 새 프로세스를 중복 실행하지 않고 기존 인스턴스를 재사용합니다. Windows는 트레이에서 창을 다시 열고, macOS Tauri 앱은 보이는 창이 없을 때 Dock 재열기 이벤트로 주 창을 복원합니다. 시스템 메모리 표시는 읽기 전용이며 누수 정리 기능이 아닙니다. 세부 경계와 플랫폼별 검증 항목은 [자동 시작과 시스템 메모리 상태 아키텍처](docs/architecture/startup-memory-status.md)에 기록합니다.
