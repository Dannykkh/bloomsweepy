...
- 개발 브랜치(출시 전 검증 중): 사용자가 설정에서 직접 등록·해제하고 실제 운영체제 상태를 다시 확인하는 Windows·macOS 자동 시작. 기본값은 꺼짐이며 자동 시작 때만 `--background`로 조용히 실행
- 개발 브랜치(출시 전 검증 중): 전체·사용 가능·사용 중 메모리와 `sysinfo`가 보고하는 swap 지표만 읽는 시스템 상태. Windows swap 값은 commit 기반 추정치이며 현재 페이지 파일 사용량이 아님. Working Set·standby 캐시·다른 앱·누수 메모리는 정리하지 않음
...
개발 브랜치의 자동 시작은 새 프로세스를 중복 실행하지 않고 기존 인스턴스를 재사용합니다. Windows는 트레이에서 창을 다시 열고, macOS Tauri 앱은 보이는 창이 없을 때 Dock 재열기 이벤트로 주 창을 복원합니다. 시스템 메모리 표시는 읽기 전용이며 누수 정리 기능이 아닙니다. 세부 경계와 출시 전 검증 항목은 [자동 시작과 시스템 메모리 상태 아키텍처](docs/architecture/startup-memory-status.md)에 기록합니다.
