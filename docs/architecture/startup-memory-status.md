...
이 문서는 `v1.5.0` Windows·macOS Tauri 앱의 자동 시작과 시스템 메모리 상태 계약을 기록합니다. Windows에서는 패키지 빌드와 숨김 시작·단일 인스턴스·창 복원 수명주기를 검증했습니다. 실제 로그인 등록 왕복과 macOS 패키지 런타임은 각 운영체제의 격리된 출시 환경에서 계속 확인해야 합니다.
